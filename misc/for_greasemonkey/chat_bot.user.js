//This script is essentially the same as "../../main.js".

// ==UserScript==
// @name YouTube Chat Bot
// @namespace youtube_chat_bot
// @description YouTube Chat Bot
// @match https://www.youtube.com/watch?v=*
// ==/UserScript==

function ____youtube_chat_bot() {
    "use strict";

    const confirmation1 = window.confirm("Would you really want to start a chat bot?");
    if (!confirmation1) {
        console.log("Cancelled the operations.");
        return;
    }

    const confirmation2 = window.confirm("Never operate a chat bot in someone else's live stream. Do you really understand what you are about to do?");
    if (!confirmation2) {
        console.log("Cancelled the operations.");
        return;
    }
    ...
    function post_chat(message = `bot test ${Math.random()}`) {
        ...
        console.log(`The chat ${message} has been posted.`);
...
function ____call_youtube_chat_bot() {
    const chat_frame = document.querySelector('#chatframe');
    if (chat_frame) {
        ____youtube_chat_bot();
    } else {
        console.log("Waiting for the chat window to be loaded...");
        window.setTimeout(____call_youtube_chat_bot, 1000);
    }
}

____call_youtube_chat_bot();
